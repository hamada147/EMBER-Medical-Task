...
public class BaseAPI<RequestModel: Codable, ResponseModel: Codable, APIErrorResponse: Codable>: APIProtocol {
    // MARK: Attributes
    ...
    private final func createRequestConfig() -> RequestConfig {
        var headerFields = self.httpHeaderFields
        headerFields.updateValue("Bearer \(APIManagerConfig.apiToken)", forKey: "Authorization")
        return RequestConfig(httpMethod: self.httpMethod, allHTTPHeaderFields: headerFields)
    }
